...
class WeatherIntegration:
    def __init__(self, config_path):
        ...
        logging.debug(f"WeatherIntegration initialized with config path: {self.config_path}")
    ...
    def load_settings(self):
        ...
        try:
            if os.path.exists(self.config_path):
                logging.debug(f"Loading weather settings from: {self.config_path}")
    ...
    def get_weather(self, force_update=False):
        ...
        logging.debug(f"Weather settings: {self.settings}")
        ...
        try:
            logging.debug("Fetching new weather data...")
            ...
            if response.status_code == 200:
                ...
                logging.debug(f"Weather data fetched successfully: {self.cached_weather}")
                return self.cached_weather
            else:
    ...
    def save_settings(self, settings):
        ...
        try:
            logging.debug(f"Saving weather settings to: {self.config_path}")
            ...
            logging.debug(f"Weather settings saved successfully to {self.config_path}")
            return True
        except Exception as e:
