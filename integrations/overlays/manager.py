...
from .qrcode import QRCodeIntegration
...
class WeatherOverlay(BaseOverlay):
    def __init__(self, weather_integration):
        ...
        self.font_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 
                                    'static', 'fonts')
        logging.debug(f"Weather overlay initialized with font path: {self.font_path}")
...
class MetadataOverlay(BaseOverlay):
    def __init__(self, metadata_integration):
        ...
        self.font_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))),
                                    'static', 'fonts')
        logger.debug(f"Metadata overlay initialized with font path: {self.font_path}")
    ...
    def apply(self, img: Image, draw: ImageDraw, image_path: str, frame=None, photo=None) -> Image:
        ...
        try:
            ...
            logger.debug("Metadata overlay successfully applied")
            return img
...
class QRCodeOverlay(BaseOverlay):
    def __init__(self, qrcode_integration):
        ...
        logger.debug("QR Code overlay initialized")
...
class OverlayManager:
    ...
    def apply_overlays(self, image_path, preferences, frame=None, photo=None):
        ...
        """
        try:
            logger.debug(f"Starting overlay application for: {image_path}")
            ...
            logger.debug(f"Using frame orientation: {desired_orientation}")
            ...
            logger.debug(f"Applying overlays with preferences: {preferences}")
                ...
                with Image.open(image_path) as img_file:
                    ...
                    try:
                        if hasattr(img_file, '_getexif') and img_file._getexif():
                            ...
                            logger.debug("Extracted EXIF data from original image")
                    except Exception as e:
            ...
            for overlay_name, overlay in self.overlays.items():
                logger.debug(f"Checking overlay: {overlay_name}")
                ...
                if preferences.get(overlay_name, False) and overlay.enabled:
                    try:
                        logger.debug(f"Applying {overlay_name} overlay")
                    ...
                    logger.debug(f"Skipping {overlay_name} overlay (enabled: {overlay.enabled}, preference: {preferences.get(overlay_name, False)})")
            ...
            if exif_data:
                ...
                logger.debug("Reattached EXIF data to modified image")
                return img_with_exif
            
            logger.debug("Overlay application completed successfully")
            return img
