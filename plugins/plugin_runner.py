...
class PluginRunner:
    ...
    def discover_and_register(self):
        ...
        for entry in os.scandir(self._plugins_dir):
            ...
            try:
                ...
                logger.info(f"Registered plugin: {plugin_id} ({class_name})")
    ...
    def load_active_plugin_jobs(self):
        ...
        logger.info(f"Loaded {len(instances)} active plugin job(s).")
    ...
    def _add_job(self, instance):
        ...
        logger.info(f"Scheduled plugin job {job_id} with cron '{instance.cron}'")
